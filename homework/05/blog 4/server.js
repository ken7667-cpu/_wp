...
app.use(session({
  secret: 'opencode-blog-secret-key-v4',
  resave: false,
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .comment-actions .btn-heart-comment .count { font-size: 0.8rem; }
    .nested-replies {
      margin-left: 52px;
      padding-left: 16px;
...
<body>
  <nav>
    <div class="nav-inner">
      ...
      <div class="nav-right">
        ${user ? `<a href="/users/${user.id}" class="username">${user.username}</a><a href="/posts/new">寫文章</a><a href="/settings">設定</a><a href="/logout">登出</a>` : `<a href="/login">登入</a><a href="/register">註冊</a>`}
...
app.get('/', async (req, res) => {
  ...
  searchHtml = `<form class="search-box" method="get" action="/"><input type="text" name="q" placeholder="搜尋文章標題..." value="${esc(q)}"><button type="submit">搜尋</button>${q ? `<a href="/" class="btn btn-sm btn-outline" style="padding:10px 16px;">清除</a>` : ''}</form>`;
  ...
  if (q) {
    ...
    if (user) {
      query = `SELECT p.*, u.username,
        ...
        WHERE (p.is_public = 1 OR p.author_id = ?) AND p.title LIKE ?
      ...
      query = `SELECT p.*, u.username,
        ...
        (SELECT COUNT(*) FROM comments WHERE post_id=p.id) as comment_count
        FROM posts p JOIN users u ON p.author_id = u.id
        WHERE p.is_public = 1 AND p.title LIKE ?
        ORDER BY p.created_at DESC`;
      params = [searchTerm];
    }
  } else if (user && tab === 'following') {
    query = `SELECT p.*, u.username,
      (SELECT COUNT(*) FROM likes WHERE target_type='post' AND target_id=p.id) as like_count,
      (SELECT COUNT(*) FROM comments WHERE post_id=p.id) as comment_count
      FROM posts p JOIN users u ON p.author_id = u.id
  ...
  const posts = db.exec(query, params);
  const rows = posts.length > 0 ? posts[0].values : [];
  const cols = posts.length > 0 ? posts[0].columns : [];

  if (rows.length === 0) {
    const msg = q ? `沒有找到與「${esc(q)}」相關的文章` : (tab === 'following' ? '追蹤中的人還沒有發表公開文章' : '目前還沒有任何文章');
    listHtml = `<div class="empty"><p>${msg}</p></div>`;
  } else {
  ...
  const heading = q ? `搜尋「${esc(q)}」的結果` : '📝 最新文章';
  const content = `<div class="card"><h1>${heading}</h1>${welcome}${searchHtml}${tabsHtml}${listHtml}</div>`;
  res.send(renderPage(q ? `搜尋：${q}` : '首頁', content, req));
});
...
app.get('/posts/:id/edit', requireAuth, (req, res) => {
  ...
  if (result.length === 0 || result[0].values.length === 0) {
    return res.status(404).send(renderPage('找不到文章', '<div class="card"><h1>404</h1><p>文章不存在</p></div>', req));
  }
  const cols = result[0].columns;
  const vals = result[0].values[0];
  const idx = {};
  cols.forEach((c, i) => idx[c] = i);
  if (vals[idx['author_id']] !== req.session.userId) {
    return res.status(403).send(renderPage('無權限', '<div class="card"><h1>🔒 無權限</h1><p>您不是作者</p></div>', req));
  }
  const checked = vals[idx['is_public']] ? 'checked' : '';
  const content = `<div class="card"><h1>✏️ 編輯文章</h1><form method="post" action="/posts/${vals[idx['id']]}/edit"><div class="form-group"><label>標題</label><input type="text" name="title" value="${esc(vals[idx['title']])}" required></div><div class="form-group"><label>內容 (支援 Markdown)</label><textarea name="content" required>${esc(vals[idx['content']])}</textarea></div><div class="form-group"><input type="checkbox" name="is_public" id="is_public" value="1" ${checked}><label for="is_public" class="checkbox-label">發布到公共貼文區</label></div><button type="submit" class="btn">儲存變更</button><a href="/posts/${vals[idx['id']]}" class="btn btn-sm btn-outline" style="margin-left:8px;">取消</a></form></div>`;
  res.send(renderPage('編輯文章', content, req));
});

app.post('/posts/:id/edit', requireAuth, (req, res) => {
  const db = getDb();
  const result = db.exec(`SELECT author_id FROM posts WHERE id=?`, [req.params.id]);
  if (result.length === 0 || result[0].values.length === 0 || result[0].values[0][0] !== req.session.userId) {
    return res.redirect('/');
  }
  const { title, content } = req.body;
  const isPublic = req.body.is_public ? 1 : 0;
  ...
  db.run(`UPDATE posts SET title=?, content=?, is_public=? WHERE id=?`, [title, content, isPublic, req.params.id]);
  saveDatabase();
...
function renderComments(postId, userId, db) {
  ...
  for (const row of rows) {
    const idx = {}; cols.forEach((c, i) => idx[c] = i);
    comments.push({
      id: row[idx['id']], post_id: row[idx['post_id']], author_id: row[idx['author_id']],
      parent_id: row[idx['parent_id']], username: row[idx['username']],
      content: row[idx['content']], created_at: row[idx['created_at']],
      like_count: row[idx['like_count']], user_liked: row[idx['user_liked']],
    });
  }
...
app.get('/posts/:id', (req, res) => {
  ...
  if (userId === post.author_id) {
    actions = `<div class="actions"><a href="/posts/${post.id}/edit" class="btn btn-sm">編輯</a><form method="post" action="/posts/${post.id}/delete" style="display:inline;" onsubmit="return confirm('確定刪除這篇文章？')"><button type="submit" class="btn btn-danger btn-sm">刪除</button></form></div>`;
  }
...
app.post('/comments/:id/reply', requireAuth, (req, res) => {
  ...
  if (!content || !content.trim()) {
    ...
    const parentResult = getDb().exec(`SELECT post_id FROM comments WHERE id=?`, [req.params.id]);
...
app.get('/settings', requireAuth, (req, res) => {
  const db = getDb();
  const result = db.exec(`SELECT bio FROM users WHERE id=?`, [req.session.userId]);
  const bio = (result.length > 0 && result[0].values.length > 0) ? (result[0].values[0][0] || '') : '';
  const error = req.session.error || '';
  const success = req.session.success || '';
  delete req.session.error;
  delete req.session.success;
  const content = `<div class="card"><h1>⚙️ 帳號設定</h1>${error ? `<div class="flash flash-error">${error}</div>` : ''}${success ? `<div class="flash flash-success">${success}</div>` : ''}<h2 style="margin-bottom:16px;">個人簡介</h2><form method="post" action="/settings" style="margin-bottom:30px;padding-bottom:20px;border-bottom:1px solid var(--border);"><div class="form-group"><label>自我介紹</label><textarea name="bio" style="min-height:80px;" placeholder>${esc(bio)}</textarea></div><button type="submit" name="action" value="bio" class="btn btn-sm">儲存簡介</button></form><h2 style="margin-bottom:16px;">變更密碼</h2><form method="post" action="/settings"><div class="form-group"><label>目前密碼</label><input type="password" name="current_password" required></div><div class="form-group"><label>新密碼</label><input type="password" name="new_password" required></div><div class="form-group"><label>確認新密碼</label><input type="password" name="confirm_password" required></div><button type="submit" name="action" value="password" class="btn btn-sm">變更密碼</button></form></div>`;
  res.send(renderPage('設定', content, req));
});
...
app.post('/settings', requireAuth, async (req, res) => {
  ...
  if (action === 'bio') {
    ...
    db.run(`UPDATE users SET bio=? WHERE id=?`, [bio, req.session.userId]);
    saveDatabase();
    req.session.success = '簡介已更新';
  ...
  if (action === 'password') {
    ...
    if (new_password !== confirm_password) {
      req.session.error = '兩次密碼輸入不一致';
      return res.redirect('/settings');
    }
    if (new_password.length < 4) {
      req.session.error = '密碼長度至少 4 個字元';
      return res.redirect('/settings');
    }
    const result = db.exec(`SELECT password FROM users WHERE id=?`, [req.session.userId]);
    if (result.length > 0 && result[0].values.length > 0) {
      const match = await bcrypt.compare(current_password, result[0].values[0][0]);
      if (!match) {
        req.session.error = '目前密碼錯誤';
        return res.redirect('/settings');
      }
    }
...
app.get('/users/:id', (req, res) => {
  ...
  cols.forEach((c, i) => idx[c] = i);
  const profile = {
    id: vals[idx['id']], username: vals[idx['username']], bio: vals[idx['bio']] || '',
...
app.listen(PORT, async () => {
  await initDatabase();
  console.log(`Blog 4 伺服器啟動於 http://localhost:${PORT}`);
  console.log(`資料庫共用 blog 3：保留原有帳號與文章`);
});
